...
uint256 hashGenesisBlock("0000004f1f40af47321360bfcfff41b95b619cb9396535abdbdc2f36d122a0e3");
...
unsigned int ComputeMinWork(unsigned int nBase, int64 nTime)
{
    ...
    if (fTestNet && nTime > nTargetSpacing*2) // No need to adjust the block time in testnet
    ...
    {
		if(nBestHeight+1 < Target_Time_Switch)
		{
		...
		{
			// Still a maximum 400% adjustment, doesn't have to be changed
			bnResult *= 4;
			nTime -= nTargetTimespan_New*4;
		}
    }
        ...
        for (unsigned int i = 1; BlockReading && BlockReading->nHeight > 0; i++) {
                ...
                PastDifficultyAveragePrev = PastDifficultyAverage;
                 ...
                 if (LatestBlockTime < BlockReading->GetBlockTime()) {
                      if (BlockReading->nHeight > 16071) // HARD Fork block number
...
unsigned int static GetNextWorkRequired_V2(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
	...
	static const int64 BlocksTargetSpacing_New = 150; // 150 seconds
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0000004f1f40af47321360bfcfff41b95b619cb9396535abdbdc2f36d122a0e3");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis block
        const char* pszTimestamp = "ZzzzzzzzZzzZzzzZZZzzzz";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 100;
        ...
        block.nVersion = 1;
        block.nTime    = 1394396785;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 5119651 ;
        ...
        assert(block.hashMerkleRoot == uint256("0x8a0be567c0968fb65ce520a74600b960d95808bcd65e5df35e5d2d940ab434e7"));
        ...
        if (false && block.GetHash() != hashGenesisBlock)
        {
            ...
            //hashTarget = bnProofOfWorkLimit.getuint256();
...
unsigned char pchMessageStart[4] = { 0xf9, 0xbe, 0xbb, 0xd2 };
...
void SHA256Transform(void* pstate, void* pinput, const void* pinit)
{
    ...
    for (int i = 0; i < 16; i++)
        ((uint32_t*)data)[i] = ReverseBytes(((uint32_t*)pinput)[i]);
...
void FormatHashBuffers(CBlock* pblock, char* pmidstate, char* pdata, char* phash1)
{
    ...
    for (unsigned int i = 0; i < sizeof(tmp)/4; i++)
        ((unsigned int*)&tmp)[i] = ReverseBytes(((unsigned int*)&tmp)[i]);
...
//            unsigned int nNonceFound;
            ...
            pblock->UpdateTime(pindexPrev);
            //nBlockTime = ReverseBytes(pblock->nTime);
            if (fTestNet)
            {
                // Changing pblock->nTime can change work required on testnet:
                //nBlockBits = ReverseBytes(pblock->nBits);
